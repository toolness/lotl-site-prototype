...
function setPageTitleFromPost(post, $) {
  // Note the title may contain character entity references.
  $('title').html(post.title_plain + ' : Life of The Law');
}

function setPageTitle(state) {
  if (state && state.post && state.post.title_plain)
    return setPageTitleFromPost(state.post, $);
...
exports.setPageTitleFromPost = setPageTitleFromPost;
