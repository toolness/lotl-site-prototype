...
var DOM_TEMPLATES = {
  results: $('#results-holder').children().remove(),
  loadingPostDetail: $('#post-detail').children().remove()
};
...
function resultsToPosts(fetchedResults, isHomepage) {
  ...
  fetchedResults.onValue(function(info) {
    ...
    if (isHomepage && ++batchesSoFar == 1)
      posts = browserUtil.reorderPostsForHomepage(posts);
...
function scrollifyPosts(qs) {
  ...
  DOM_TEMPLATES.results.clone().appendTo(holder);
...
function showPostDetail(slug) {
  ...
  var rendered = $('<div></div>')
    .append(DOM_TEMPLATES.loadingPostDetail.clone());
...
$(function() {
  ...
  queries.skipDuplicates().onValue(function(q) {
    var href = '/' + (q ? '?s=' + encodeURIComponent(q) : '');
    if (window.location.pathname + window.location.search != href)
      window.history.pushState({}, '', href);
    if (q == 'podcast') return scrollifyPosts({category_name: q});
    if (q in TAGS)
      return scrollifyPosts({tag_id: TAGS[q]});
    scrollifyPosts();
  });
  ...
  $(window).on('popstate', function(event) {
    ...
    var search = querystring.parse(window.location.search.slice(1)).s;

    queries.push(search || null);
