...
function createHomepageResultBuilder() {
  var batchesSoFar = 0;
  var postsSoFar = 0;

  return {
    resultsForPage: createResultsForPage({}),
...
function createResultBuilder(q, tags) {
  ...
  if (q == 'podcast') {
    builder = createGenericResultBuilder({category_name: q});
  } else if (q in tags) {
    builder = createGenericResultBuilder({tag_id: tags[q]});
  } else {
    builder = createHomepageResultBuilder();
  }

  return $.extend({
    resultsToPosts: resultsToPosts,
