...
function getThumbnail(headshot, imageIndex) {
  ...
  var height = parseInt(headshot.attr('height') || '0');
  var width = parseInt(headshot.attr('width') || '0');
  var entry = imageIndex[src];

  if (entry && entry.best &&
      height <= entry.best.height && width <= entry.best.width) {
    return entry.best;
  }
  return {
    url: src,
    ...
    resultsToPosts: function(fetchedResults) {
      ...
      fetchedResults.onValue(function(info) {
        ...
        things.each(function() {
          ...
          var nameAndTitle = $.trim($('strong', this).text());
          var post = {
